...
def Lgb_train_and_predict(train, test, config, gkf=False, aug=None, output_root="./output/", run_id=None):
    # output_pathの作成 --
    if not run_id:  # run_id==NoneでTrue --
        # 基本は時間をidにするけど、1秒以内に連続して実行したらエラーになることの回避 --
        run_id = "run_lgb_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        while os.path.exist(output_root+run_id+"/"):
            time.sleep(1)
            run_id = "run_lgb_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    os.system(f"cp ./*.py {output_path}") # これやばくね？S1~S7まで全部行くやろ -- 
    os.system(f"cp ./*.sh {output_path}") # => たぶんこのスクリプト回した時点の特徴量作るpyとかを保存したいってことだと思った--
    ...
    if train is not None:
        ...
        if gkf:
            # 重複customer_IDをdrop, customer_IDにつきtargetは一意に定まるので
            # customer_ID - targetのペアdfがtmpに保存される --
        ...
        for fold, (trn_index, val_index) in enumerate(split):
            ...
            for i in range(len(evals_result_dic["valid_1"][params["metric"]])//verbose): # evals_result_dicの中身分からない問題 --
